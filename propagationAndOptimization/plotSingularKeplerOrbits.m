...
%%
for k=1:3
    dataFolder = 'singlePerturbedSatellitePropagatPropagationResults/';
...
for k=1:4
    dataFolder = 'singlePerturbedSatellitePropagatPropagationResults/';
    perturbedSatelliteUsm = load(strcat(dataFolder,'singleUnperturbedSatellitePropagationUsmHistorySingular',num2str(k-1),'.dat'));
    time = ( perturbedSatelliteUsm(:,1) - perturbedSatelliteUsm(1,1) )/3600;
    ...
    for i=1:7
        ...
        subplot(2,4,i+1)
        plot(time,multiplier*perturbedSatelliteUsm(:,i+1),'LineWidth',2)
        xlim([0 24])
        xlabel('t [hr]','interpreter','latex')
        grid on
%         if( i == 1)
